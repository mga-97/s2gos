...
def run_pipeline(scene_config_path: Path, sim_config_path: Optional[Path] = None):
    ...
    try:
        ...
        print(f"  Target: {scene_gen_config.location.aoi_size_km}km² at {scene_gen_config.processing.target_resolution_m}m")
        if scene_gen_config.has_buffer:
            print(f"  Buffer: {scene_gen_config.buffer.buffer_size_km}km² at {scene_gen_config.buffer.buffer_resolution_m}m")
        print(f"  Output: {scene_gen_config.scene_output_dir}")
    ...
    if simulation_config and ERADIATE_AVAILABLE and scene_description:
        ...
        try:
            ...
            print("   This might be expected if eradiate is not available or not configured properly")
            return False
